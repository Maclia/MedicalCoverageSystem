...
export const rateLimitConfigurations = {
  ...
  // Strict rate limit for sensitive billing operations
  strict: {
    windowMs: 15 * 60 * 1000, // 15 minutes
  ...
  // Very strict rate limit for payment processing
  payments: {
    windowMs: 15 * 60 * 1000, // 15 minutes
    max: 20, // 20 payment requests per 15 minutes
    message: 'Too many payment attempts, please try again later.',
    skipSuccessfulRequests: true // Don't count successful payment attempts
  },

  // Rate limit for commission calculations
  commissions: {
    windowMs: 60 * 60 * 1000, // 1 hour
    max: 100, // 100 commission calculations per hour
    message: 'Too many commission calculations, please try again later.'
  },

  // Rate limit for invoice operations
  invoices: {
    windowMs: 15 * 60 * 1000, // 15 minutes
    max: 100, // 100 invoice operations per 15 minutes
    ...
    if (req.user) {
      switch (req.user.role) {
        case 'admin':
          ...
          break;
        case 'billing':
          // High limits for billing staff
          options = {
            windowMs: 15 * 60 * 1000,
            max: 1500,
            message: 'Billing staff rate limit exceeded',
            keyGenerator: (req) => `billing:${req.user?.id}`
          };
          break;
        default:
