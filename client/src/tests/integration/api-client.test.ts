...
describe("Enhanced Members API Client Integration Tests", () => {
  ...
  afterAll(() => {
    ...
    if (testSuite.failed > 0) {
      console.log("\nFailed Tests:");
      testSuite.tests.filter(t => !t.passed).forEach(t => console.log(`- ${t.testName}: ${t.error}`));
    }
  });

  describe("Member Management API", () => {
    it("Create member with enhanced fields", async () => {
    ...
    it("Search members with enhanced filters", async () => {
      const test = await TestRunner.runTest("Search members with enhanced filters", async () => {
        const searchParams = {
          searchTerm: "John",
          gender: "male",
          hasDisability: false,
  ...
  describe("Document Management API", () => {
    it("Upload member document", async () => {
      const test = await TestRunner.runTest("Upload member document", async () => {
        ...
        const result = await membersAPI.uploadDocumentFile(1, file, documentData.documentType, documentData.documentName);

        // NOTE: Real implementation would build FormData internally. We assert header token presence only.
  ...
  describe("Consent Management API", () => {
    it("Update member consent", async () => {
      const test = await TestRunner.runTest("Update member consent", async () => {
        const consentData = {
          memberId: 1,
    ...
    it("Bulk consent operations", async () => {
      const test = await TestRunner.runTest("Bulk consent operations", async () => {
  ...
  describe("Bulk Operations API", () => {
    it("Bulk member creation", async () => {
      const test = await TestRunner.runTest("Bulk member creation", async () => {
        const bulkRequest = {
          companyId: 1,
          members: [
  ...
  describe("Error Handling", () => {
    it("API error handling", async () => {
      const test = await TestRunner.runTest("API error handling", async () => {
    ...
    it("Network error handling", async () => {
      const test = await TestRunner.runTest("Network error handling", async () => {
  ...
  describe("Performance Tests", () => {
    it("API response time should be under 1000ms", async () => {
      const performanceTest = PerformanceTest.createPerformanceTest(
        "API response time should be under 1000ms",
        1000
      );

      const test = await performanceTest(async () => {
        const mockResponse = {
        ...
        await membersAPI.getMembers({ page: 1, limit: 50 } as any);
      });
  ...
  describe("Dashboard Statistics API", () => {
    it("Get dashboard statistics with enhanced metrics", async () => {
